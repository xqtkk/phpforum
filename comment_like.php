...
if ($existing === false) {
    $pdo->prepare("INSERT INTO comment_reactions (user_id, comment_id, reaction) VALUES (?, ?, 'like')")
...
elseif ($existing === "dislike") {
    $pdo->prepare("UPDATE comment_reactions SET reaction='like' WHERE user_id=? AND comment_id=?")
        ->execute([$user, $comment]);
